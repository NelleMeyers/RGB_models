...
PDM_validation<-function(Overall_dataset){
  ...
  B <-as.factor(RGB_validation_data$Particle_type)
...
Import<-setwd("Location_of_RGB-datasets_with_unknown_particles_which_are_to_be_identified")
Export <-setwd("Location_where_particle_prediction_xlsx-file_should_be_stored")

#PDM
PDM_prediction<-function(Location_unknown_datasets){
  traindataset<-create_train_test(RGB_dataset, size = 0.8, train=TRUE)
  model<-PDM_creation(traindataset)  
  files<-list.files(path = Location_unknown_datasets, pattern=".xlsx")
  for (f in files) {
    Prediction_df<-read.xlsx( paste0(Import,f), sheetIndex = 1)
    ...
    for (i in 1:nrow(Prediction_df)){
      subdataset<-Prediction_df[2:37]
      predicted_ID<-predict(model,subdataset[i,],type='class')
      newrow=c(Prediction_df[i,1],predicted_ID)
...
#PIM
PIM_prediction<-function(Location_unknown_datasets){
  traindataset<-create_train_test(RGB_dataset, size = 0.8, train=TRUE)
